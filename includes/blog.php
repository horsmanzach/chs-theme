...
class custom_ET_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
	...
	echo '<div class="post-content">';
					...
					/*Zoom Link Custom Field*/

					$zoom_link = get_post_meta($post->ID, 'zoom_link', true);
						if ($zoom_link != '') {
					...
					if ($date_of_event) {
    					$date = DateTime::createFromFormat('Ymd', $date_of_event);

    					// Format each part of the date
    					$day_name = $date->format('D'); // e.g., "Thu"
    					$day_number = $date->format('j'); // e.g., "12"
    					$month_year = $date->format('M Y'); // e.g., "Sep 2024"

    					// Output the formatted date with separate classes for each part
   						 echo '<div class="date-of-event">';
    					echo '<div class="event-day-name">' . esc_html($day_name) . '</div>';
   						 echo '<div class="event-day-number">' . esc_html($day_number) . '</div>';
